...
extension VoiceActivityDetector {
    ...
    public func detect(sampleBuffer: CMSampleBuffer,
        ...
        guard let dataBuffer = CMSampleBufferGetDataBuffer(sampleBuffer) else { // CMBlockBuffer
            return nil
        }

        let sampleBytes = CMSampleBufferGetSampleSize(sampleBuffer, at: 0) // Int
        ...
        let ret = CMBlockBufferGetDataPointer(dataBuffer,       // OSStatus
        ...
        let sampleCount = bufferBytes / 2 // Int
        return bufferPointer!.withMemoryRebound(to: Int16.self, capacity: sampleCount) { (buffer) in
            ...
            var presentationTimestamp = CMSampleBufferGetPresentationTimeStamp(sampleBuffer)  // CMTime
